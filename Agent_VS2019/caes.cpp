...
BYTE SBox[16][16] = {
    {0x63, 0x7c, 0x77, 0x7b, 0xf2, 0x6b, 0x6f, 0xc5, 0x30,0x01, 0x67, 0x2b, 0xfe, 0xd7, 0xab, 0x76},
                     ...
                     {0x8c, 0xa1, 0x89, 0x0d, 0xbf, 0xe6, 0x42, 0x68, 0x41,
                      0x99, 0x2d, 0x0f, 0xb0, 0x54, 0xbb, 0x16} };  // s-盒
...
BYTE Rc[31] = { 0x00, 0x01, 0x02, 0x04, 0x08, 0x10, 0x20, 0x40, 0x80, 0x1b, 0x36,
...
void print_allval() {
    for (int j = 0; j < sizeof(State); j++) {
        for (int i = 0; i < 4; i++) {
            printf("State: %02x %02x %02x %02x \n", *(State + j)[4 * i + 0], *(State + j)[4 * i + 1], *(State + j)[4 * i + 2], *(State + j)[4 * i + 3]);
            printf("w: %02hhn %02hhn %02hhn %02hhn \n", w[4 * i + 0], w[4 * i + 1], w[4 * i + 2], w[4 * i + 3]);
            printf("key: %02hhn %02hhn %02hhn %02hhn \n", key[4 * i + 0], key[4 * i + 1], key[4 * i + 2], key[4 * i + 3]);
        }
    }
}


void cerate_CAES()  // type=true時為加密文件,type=false時為加密字符串
{
    int i;
    for (i = 0; i < 4; i++)  //置空
    {
...
bool SetKeys(int KeySize, const char* sKey) {
    ...
    {
        key[i] = (BYTE*)malloc(Nk * sizeof(BYTE));
        if (key[i] == NULL) {
            return false;
        }
    }
    ...
    {
        w[i] = (BYTE*)malloc(4 * (Nr + 1) * sizeof(BYTE));
        if (w[i] == NULL) {
            return false;
        }
    }
    for (i = 0; i < 4; i++)  //獲得密鑰
        for (j = 0; j < Nk; j++) key[i][j] = (BYTE)sKey[Nk * i + j];
    // key[i][j]=(byte)sKey.GetAt(Nk*i+j);
    KeyExpansion();  //生成密鑰調度表
    return true;
}

void SubBytes()  //字節代替變換
{
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
...
void ShiftRows()  //循環移位
{
    ...
    for (int r = 1; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
            State[r][c] = temp[r].w[(c + r) % 4];
        }
    }
}

void MixColumns()  //列混合變換
{
    struct BYTE4 temp[4];
    for (int r = 0; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
...
void AddRoundKey(int round)  //輪密鑰加法變換
{
    int i, j, k = round * 4;
    for (i = 0; i < 4; i++)
        for (j = 0; j < 4; j++) State[i][j] = State[i][j] ^ w[i][k + j];
}

void KeyExpansion()  //密鑰擴展
{
    BYTE temp[4], tp;
    int i, j, x;
    ...
    while (i < 4 * (Nr + 1))  //生成後幾個字
    {
        ...
        if (i % Nk == 0) {
            ...
            for (j = 0; j < 4; j++)
                temp[j] = SBox[(temp[j] >> 4) & 0x0F][temp[j] & 0x0F];
        for (j = 0; j < 4; j++) w[j][i] = (w[j][i - Nk]) ^ temp[j];
        i++;
    }
    for (int k = 0; k <= Nr;
        k++)  //實現密鑰的每16個字節為一個矩陣的矩陣轉置(對字符串加密/解密有影響)
        for (i = 0; i < 3; i++) {
            x = 4 * k;
            for (j = i + 1; j < 4; j++) {
                tp = w[i][x + j];
...
void InvShiftRows()  //循環右移函數(0000000000)
{
    struct BYTE4 temp[4];
    for (int r = 0; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
    ...
    for (int r = 1; r < 4; ++r) {
        for (int c = 0; c < 4; ++c) {
            State[r][(c + r) % 4] = temp[r].w[c];
        }
    }
}

void InvSubBytes()  //利用逆S盒InvSBox實現逆字節代替變換
{
    for (int i = 0; i < 4; i++)
        for (int j = 0; j < 4; j++)
...
void Encrypt(BYTE* input, BYTE* output) {
    BYTE in[4][4];
    BYTE* temp = &(in[0][0]);
    for (int i = 0; i < 16; i++) *(temp + i) = *(input + i);
    State = in;
    ...
    for (int n = 0; n < 16; n++) *(output + n) = *(temp + n);
}
...
void EncryptBuffer(BYTE* input, int length) {
    ...
    if (count > 0) {
        ...
        for (int i = 0; i < count * 16; i += 16) {
            for (n = 0; n < 16; n++) pin[n] = *(input + (i + n));
            Encrypt(pin, pout);
            for (n = 0; n < 16; n++) *(input + (i + n)) = pout[n];
        }
        if (len > 0) {
            int k;
            for (int n = 0; n < 16 - len; n++) pin[n] = pout[len + n];
            k = 0;
...
void EncryptString(BYTE* input) {
    ...
    EncryptBuffer(reinterpret_cast<BYTE*>(input), length);
}
...
void Decrypt(BYTE* input, BYTE* output) {
    ...
    for (int i = 0; i < 16; i++) *(temp + i) = tempinput[i];
    State = in;
...
void DecryptString(BYTE* input) {
    size_t length = strlen((const char*)input);
    DecryptBuffer(reinterpret_cast<BYTE*>(input), length);
}
...
BYTE GfCalc(BYTE b, int Mode) {
    ...
    case 3:  //(b*2*2*2)+b*1
        return (BYTE)((int)GfCalc(GfCalc(GfCalc(b, MODE02), MODE02), MODE02) ^
            (int)b);
        break;
    case 4:  //(b*2*2*2)+(b*2)+b*1
        return (BYTE)((int)GfCalc(GfCalc(GfCalc(b, MODE02), MODE02), MODE02) ^
            (int)GfCalc(b, MODE02) ^ (int)b);
        break;
    case 5:  //(b*2*2*2)+(b*2*2)+b*1
        return (BYTE)((int)GfCalc(GfCalc(GfCalc(b, MODE02), MODE02), MODE02) ^
            (int)GfCalc(GfCalc(b, MODE02), MODE02) ^ (int)(b));
        break;
    case 6:  //(b*2*2*2)+(b*2*2)+b*2
        return (BYTE)((int)GfCalc(GfCalc(GfCalc(b, MODE02), MODE02), MODE02) ^
...
// void SayHello(const char* input) {
//   printf("Hello %s\n", input);
// }

// extern "C" EXPORTED_SYMBOL 
// void PassPacket(const StrPacket *packet) {
//   printf("packet %p\n", &packet);
//   printf("MAC:%s, IP:%s, DoWorking:%s, Message:%s\n", packet->MAC, packet->IP, packet->DoWorking, packet->Message);
// }
...
//                                              BYTE **data) 
// {
//     /*printf("AESKey: %s, MAC:%s, IP:%s, DoWorking:%s, Message:%s\n", aesKey, 
//          packet->MAC,
//          packet->IP,
//          packet->DoWorking, 
//          packet->Message);*/

//      size_t data_size = sizeof(BYTE) * 1024;
//      *data = (BYTE*)malloc(data_size);
//      memcpy(*data, packet, data_size);
//      //*data = (BYTE *)packet;

//     /* printf("copy packet data: ");
...
//     aes->EncryptBuffer((BYTE*)*data, 1024);
...
//    CAES *aes = new CAES();
//    aes->SetKeys(BIT128, aesKey);
//    aes->DecryptBuffer((BYTE *)data, 1024);

//    memcpy(packet, (StrPacket *)data, sizeof(BYTE) * 1024);

//    /*printf("AESKey: %s, MAC:%s, IP:%s, DoWorking:%s, Message:%s\n", aesKey,
//           packet->MAC, packet->IP, packet->DoWorking,
//           packet->Message); */
//    delete aes;
// }