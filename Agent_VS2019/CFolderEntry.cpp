...
bool CFolderEntry::load(void *argv,char* pMAC,char* pIP,DWORD FatherID,char * SendStr,unsigned int & pProgressCount,unsigned int & pCount,DWORD & pLastCluster,vector<DeleteFATFileInfo>* pDelInfo,clock_t & pStartTime)
{
	//TransportData * m_Client = (TransportData *)argv;
	//DWORD dwChainedClustersSizeBytes = 0;
	//// First - Gets the size of the data
	...
	//							if(!newEntry->load(argv,pMAC,pIP,FirstCluster,SendStr,pProgressCount,pCount,pLastCluster,pDelInfo,pStartTime))
	//								return false;
	//						}
	//					}
	//					else
