...

{
	...
	if (m_hDevice == INVALID_HANDLE_VALUE)//if (FAILED(hDevice))
	{
...
void CVolumeAccess::initData()
{
	...
	#else
	DISK_GEOMETRY lDiskGeo;
	if (DeviceIoControl(m_hDevice, IOCTL_DISK_GET_DRIVE_GEOMETRY, NULL, 0, &lDiskGeo, sizeof(DISK_GEOMETRY), &lBytes, NULL))
	{
		m_sectorSize = lDiskGeo.BytesPerSector;
		//_tprintf_s(_T("Sector size is %d bytes\n"), m_sectorSize);
	}
	#endif
...
void CVolumeAccess::dumpFatsData(TCHAR* aDestPath)
{
	...
	int iSize = (int)(_tcslen(aDestPath) + _tcslen(szFileName1) + 1);
	...
	iSize =  (int)(_tcslen(aDestPath) + _tcslen(szFileName2) + 1);
...
DWORD CVolumeAccess::getClusterSize()
{
	return m_clusterSizeBytes;
}
